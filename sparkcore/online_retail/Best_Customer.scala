...
object Best_Customer {
  def main(args: Array[String]) {
    ...
    val retaildf= sqlContext.read.format("com.databricks.spark.csv").option("header", "true").load(data)
    //val retailrdd = sc.textFile(data)
    //val head= retailrdd.first()
    //val fil = retailrdd.filter(x=>x!=head).map(x=>x.split(","))
    //val fil = retailrdd.filter(x=>x!=head).map(x=>x.split(",")).map(x=>x(0).distinct)
